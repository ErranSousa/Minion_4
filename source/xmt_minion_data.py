...
def default_data_xmt_status_dict(data_xmt_status_dict):
    # default_timestamp = [0, 0, 0, 0, 0, 0]
    # # Write the default time stamp values to the first_gps_time_dict
    # for key_1, val_1 in zip(data_xmt_status_dict['first_gps_time_dict'], default_timestamp):
    #     data_xmt_status_dict['first_gps_time_dict'][key_1] = val_1
...
# create an empty dictionary with empty values - note: first_gps_time is a dict
keys = ['first_gps_time_dict', 'num_gps_sent', 'all_files_transmitted', 'curr_file_name', 'file_open_success',
...
# create an empty dictionary for the first gps timestamp and assign it to a key in the data_xmt_status_dict
keys_timestamp = ['YYYY', 'MM', 'DD', 'hh', 'mm', 'ss']  # Year, Month, Day, Hours, Minutes, Seconds
data_xmt_status_dict['first_gps_time_dict'] = dict.fromkeys(keys_timestamp)
...
try:
    ...
    data_xmt_status_dict['all_files_transmitted'] = False
    data_xmt_status_dict['curr_file_name'] = ''
    # Assign the current time to the first
    data_xmt_status_dict['first_gps_time_dict'] = minion_tools.rtc_ext_get_time()
    default_data_xmt_status_dict(data_xmt_status_dict)  # default states for all the other members
...
if XMT_INI:  # If configured to transmit the Initial Mode Data File
    fnames_with_paths = fnames_with_paths + glob.glob(minion_data_config_dict['Data_Dir'] +
                                                      '/minion_data/INI/*_TEMPPRES-INI.txt')
if XMT_TLP:  # If configured to transmit the Time-Lapse Mode Data Files
    fnames_with_paths = fnames_with_paths + glob.glob(minion_data_config_dict['Data_Dir'] +
                                                      '/minion_data/*_TEMPPRES.txt')
if XMT_FIN:  # If configured to transmit the Final Mode Data File
    fnames_with_paths = fnames_with_paths + glob.glob(minion_data_config_dict['Data_Dir'] +
                                                      '/minion_data/FIN/*_TEMPPRES-FIN.txt')
...
# **************************************************************************************
# ****** Read and convert the timestamps for determining if to send GPS records ******
# get the current time from the external RTC
now_time = minion_tools.rtc_ext_get_time()
# convert it to datetime format
now_time_datetime = datetime(int(now_time['YYYY']), int(now_time['MM']), int(now_time['DD']),
                             int(now_time['hh']), int(now_time['mm']), int(now_time['ss']))
# get the time of the first entry into this script
start_time = data_xmt_status_dict['first_gps_time_dict']
...
#         not detect_data_files_flag:
#     print('Sending GPS !!!')
# else:
#     print("Sending Data !!!")
# **************************************************************************************

# exit(0)  # get out, just for testing
...
# Transmit Data via the Iridium Constellation
    ...
    if data_xmt_status_dict['all_files_transmitted']:
    ...
    for idx, val in enumerate(fnames):
        ...
        # Start sending from the current file name as listed in the dictionary / pickle file
            ...
            while not data_xmt_status_dict['xmt_file_complete'] and num_tries < MAX_TRIES:
                print("\n\r")
                print(val)
                ...
                if not data_xmt_status_dict['xmt_file_complete']:
            ...
            # ---------------------------------------------------------------------------------------- #
            # update the dictionary & the pickle file with the next file name to transmit
            ...
            # the list.
            elif data_xmt_status_dict['xmt_file_complete'] and idx + 1 <= len(fnames) - 1:
